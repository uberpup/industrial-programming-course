...
void ImStack<T>::InitCanaries() {
    buf[0] = std::hash<T>(CANARYVALUE_S);
    buf[capacity - 1] = std::hash<T>(CANARYVALUE_E);
}
...
bool ImStack<T>::OK() {
    ...
    if (buf[0] != std::hash<T>(CANARYVALUE_S)) {
        return false;
    }
    if (buf[capacity - 1] != std::hash<T>(CANARYVALUE_E)) {
        return false;
    }
    for (size_t i = sz; i < capacity; ++i) {
        if (buf[i] != std::hash<T>(POISON)) {
            return false;
        }
    }
    return true;
